...
class GaussianMLPPolicy(nn.Module):
    ...
    def forward(self, state, goal):
        x = torch.cat([state, goal], dim=-1)  # [B, input_dim]
        h = self.trunk(x)                     # [B, hidden_dims[-1]]
        mean = self.mean_head(h)              # [B, action_dim]
        std = self.log_std.clamp(self.log_std_min, self.log_std_max).exp().expand_as(mean)  # [action_dim] -> [B, action_dim]
        return torch.distributions.Normal(mean, std)

    def get_action_mean(self, state, goal):
        x = torch.cat([state, goal], dim=-1)  # [B, input_dim]
        h = self.trunk(x)                     # [B, hidden_dims[-1]]
        return self.mean_head(h)              # [B, action_dim]

    @torch.no_grad()
    def act(self, state, goal, deterministic=False):
        dist = self.forward(state, goal)
        action = dist.mean if deterministic else dist.sample()  # [B, action_dim]
...
class CNNGaussianPolicy(nn.Module):
    ...
    def __init__(self, action_dim, frame_stack=1, encoder_size=18,
        ...
        # Frozen ResNet encoder (ImageNet-pretrained, standard 3-channel input)
        # Use penultimate features (before classification head) for richer representations
        if encoder_size == 18:
        ...
        self.encoder.fc = nn.Identity()  # drop classification head, use backbone features
    ...
    def encode(self, img):
        """Encode stacked frames independently. img: [B, 3*fs, H, W] float [0-255]."""
        x = img.float() / 255.0 if img.max() > 2.0 else img  # [B, 3*fs, H, W]
        B = x.shape[0]
        frames = x.unfold(1, 3, 3).permute(0, 1, 4, 2, 3)    # [B, fs, 3, H, W]
        frames = frames.reshape(B * self.frame_stack, 3, x.shape[2], x.shape[3])  # [B*fs, 3, H, W]
        frames = self.preprocess(frames)                       # [B*fs, 3, 224, 224]
        embs = self.encoder(frames)                            # [B*fs, encoder_dim]
        return embs.reshape(B, -1)                             # [B, fs * encoder_dim]

    def forward(self, img_s, img_g):
        emb_s = self.encode(img_s)                             # [B, fs * encoder_dim]
        emb_g = self.encode(img_g)                             # [B, fs * encoder_dim]
        x = torch.cat([emb_s, emb_g], dim=-1)                 # [B, 2 * fs * encoder_dim]
        h = self.trunk(x)                                      # [B, hidden_dims[-1]]
        mean = self.mean_head(h)                               # [B, action_dim]
        std = self.log_std.clamp(self.log_std_min, self.log_std_max).exp().expand_as(mean)  # [B, action_dim]
        return torch.distributions.Normal(mean, std)
    ...
    def get_action_mean(self, img_s, img_g):
        ...
        emb_g = self.encode(img_g)                             # [B, fs * encoder_dim]
        x = torch.cat([emb_s, emb_g], dim=-1)                 # [B, 2 * fs * encoder_dim]
        return self.mean_head(self.trunk(x))                   # [B, action_dim]
